...
class Point(object):
    ...
    def __init__(self,x,y):
        """__init__ gives values to the instance variables"""
        self.x=x
        self.y=y
        self.move=0
        self.initialx=x
    ...
    def get_distance_from_start(self):
        return self.get_distance_from(Point(self.initialx,self.initialy))
    def get_distance_traveled(self):
        total=0
        d=self.get_distance_from_start()
        total=total+d
        return total
    def closer_to(self,p2,p3):
        a=self.get_distance_from(p2)
        b=self.get_distance_from(p3)
        if a<=b:
            return p2
        else:
            return p3
